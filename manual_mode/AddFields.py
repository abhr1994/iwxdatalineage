...
def main():
    ...
    with open(args["add_fields_csv_path"], mode='r') as file:
        ...
        DataTypes = {"integer": int, "float": float, "string": str, "boolean": bool, "bytes": bytes, "array": list,
        ...
        for row in fieldsFile:
            if row:
                ...
                if datastore:
                    logging.info("Got connected to Datastore {datastore_name} Successfully".format(
                        datastore_name=datastore_name))
                else:
                    logging.error(
                        "Failed to connect to Datastore {datastore_name}".format(datastore_name=datastore_name))
